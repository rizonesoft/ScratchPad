...
Validation (fails loudly): an unparseable line or a JSON-RPC envelope violation (missing `jsonrpc: "2.0"`, missing `method`) is reported on stderr and the process exits 2 without writing stdout garbage. A well-formed request with invalid params (`initialize` without numeric `protocolVersion`, `session/new` without string `cwd`, `session/prompt` without string `sessionId` and array `prompt`) gets a JSON-RPC error response (`-32602`) plus a stderr note; an unknown method gets `-32601`.
