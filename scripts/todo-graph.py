...
# §1 | T02 §3 | D02 T01 §4
# The trailing boundary (D00 T01 §34 R1 consistency 1): without it a
# 10-digit reference silently truncates to its first 9 digits and
# misquotes in diagnostics, so over-long runs match nothing here and
# the companion pattern below names them whole instead.
XREF_RE = re.compile(r"(?:D(?P<dom>\d{2})\s+)?(?:T(?P<todo>\d{2})\s+)?§(?P<sec>\d{1,9})(?!\d)")
OVERLONG_REF_RE = re.compile(r"§\d{10,}")
...
# waiver voids on a stamp that changed nothing. Closed gap (D00
# T04 §6 item 3): a lifted reopen leaves no trace, so before the
# stamp-history leg a hand edit stripping `generation <n>` off a
# post-lift stamp (or a re-verification forgetting it) revived
# old waivers silently. Generations are monotonic
# non-decreasing per section against HEAD now; only a deliberate
# history rewrite reaches past the leg.
...
SEVERITY_MAP: dict[str, str] = {
    ...
    "stamp-generation-regressed": "fatal",
...
def _blank_register_state(text: str) -> str:
    ...
    normalize, and line endings plus the terminal newline pass
    through byte-identical (D00 T01 §34 R1 record 2: a splitlines
    rebuild would also swallow newline drift), so foreign lines and
    ending drift still fail the check."""
    out = []
...
def evidence_lineage(evi: str, candidate: str | None) -> str:
    ...
    reports proven foreignness. Peel-identity is deliberately NOT
    required (D00 T04 §6 item 2 decides): the normative test
    asked for a reachable scenario where tag-object evidence
    attests a state the reviewed candidate cannot see, and none
    exists -- the peeled evidence either is the peeled candidate
    or descends from it (visible), or the foreign leg reports it;
    tag-ness is unobservable past the peel.
    """
...
NONCOVER_FIX = {
    ...
    "terminal-outcome": "supersede with a live record or remediate the finding",
}
...
# Health findings as named results (D00 T04 §6 item 5): the
# criticals/majors positional tuples whose swap item 1 repaired
# are these two NamedTuples instead, so same-typed fields
# cannot silently reorder again. Producers construct with
# keywords, consumers read attributes; field names match the
# plan-health JSON keys exactly (majors `since` included), and
# the self-test pins the field order plus that alignment.
class CriticalFinding(NamedTuple):
    file: str
    id: str
    owner: str
    due: str
...
def stamp_generations_by_section(text: str) -> dict[int, int]:
    """Persisted stamp generations per section (D00 T04 §6 item
    3): the last `Verified:` line's generation per covered
    section, ranges expanded, absent generation reading 0. The
    ...
    for ln in text.splitlines():
        ...
        if sm is None or sm.group("kind") != "Verified":
            continue
        shaped = STAMP_BODY_RE.match(sm.group("body").strip())
        if shaped is None or not shaped.group("evidence"):
            continue
...
# optional backticks, 1-4 non-word separators, verdict with a word
# boundary, trailing prose allowed.
TELEMETRY_VERDICT_RE = re.compile(
    r"^[ \t]{0,3}[*`>\-][ *`>\-]*`?(adversarial|consistency|integration|record|source-defect|design)`?[^\w\n]{1,4}"
...
def telemetry_parse(text: str) -> dict:
    ...
    for ln in text.splitlines():
        ...
        if ln.startswith("Telemetry:"):
            ...
            tel = None
            if m is not None and cur is not None and cur["telemetry"] is None:
                tok = m.group(6).replace(",", "").strip()
                dur = m.group(4).strip()
                tok_v = int(tok) if re.fullmatch(r"\d+", tok) else None
                dur_v = int(dur[:-1]) if re.fullmatch(r"\d+s", dur) else None
                if (tok_v is not None or tok.lower() == "unknown") and (
                    dur_v is not None or dur.lower() == "unknown"
                ):
                    tel = {
                        "round": int(m.group(1)),
        ...
        if vm and cur is not None:
            cur["verdicts"].append((vm.group(1), vm.group(2)))
            continue
...
def cmd_query(args) -> int:
    ...
    if what == "telemetry":
        ...
        def _read_findings(path: str) -> str | None:
            try:
                raw = (TODO_DIR.parent / path).read_text(encoding="utf-8")
            except OSError:
                return None
            txt, _u = strip_fenced_code(raw)
            return txt
        ...
        if target:
            if Path(target).is_file():
                try:
                    ...
                    print(f"telemetry: cannot read {target}")
                    return 1
                txt, _u = strip_fenced_code(raw)
                files[target] = txt
                section_mode = True
                ...
                res = (
                    resolve_ref(target, todos[0], by_key)
                    if todos and re.search(r"D\d+\s*T\d+\s*§\d+", target)
                    else None
                )
                if res is None:
                    print(f"unknown telemetry target: {target} (give D00 T01 §N or a findings path)")
                    return 1
                tid, snum = res
                tobj = next((t for t in todos if t.id == tid), None)
                sec = tobj.sections.get(snum) if tobj else None
                fm = FINDINGS_RE.search(getattr(sec, "review_body", None) or "") if sec else None
                if not fm:
                    print(f"telemetry: {target} names no findings file")
                    return 1
                txt = _read_findings(fm.group(1))
                if txt is None:
                    print(f"telemetry: cannot read {fm.group(1)}")
                    return 1
            ...
            for t in todos:
                for _num, _s in t.sections.items():
                    _fm = FINDINGS_RE.search(getattr(_s, "review_body", None) or "")
                    if not _fm or _fm.group(1) in files:
                        continue
                    _txt = _read_findings(_fm.group(1))
                    if _txt is not None:
        ...
        def _one_section(path: str, d: dict) -> tuple[list[str], dict]:
            ...
            for _i, _dp, _ev, _rn in d["disps"]:
                _tag = f"R{_rn}" if _rn is not None else "R?"
                chains.setdefault(_i, []).append(f"{_tag}:{_dp}({_ev})")
                finals[_i] = (_dp, _ev)
            blockers = sorted(i for i, (dp, _e) in finals.items() if dp == "live")
            filed = sorted(i for i, (dp, _e) in finals.items() if dp == "filed")
            ...
            if filed:
                lines.append("filed follow-ups: " + ", ".join(f"{i} -> {finals[i][1]}" for i in filed))
            else:
                lines.append("filed follow-ups: none")
            if chains:
                lines.append("lineage: " + "; ".join(f"{i}: {' > '.join(chains[i])}" for i in sorted(chains)))
            else:
                lines.append("lineage: none")
            lines.append(f"attestor: {att_txt}")
            _mm = sum(
                1 for r in d["rounds"] if r["telemetry"] and r["telemetry"]["round"] != r["n"]
            )
            if _mm:
                lines.append(f"round/telemetry mismatches: {_mm} (telemetry round field disagrees with its section)")
            if d["malformed"]:
                lines.append(f"malformed telemetry lines skipped: {len(d['malformed'])}")
            jd = {
                "ref": section_ref or path,
                "path": path,
                "rounds": [
                    {
                        "n": r["n"],
                        ...
                        "verdicts": [list(v) for v in r["verdicts"]],
                    }
                    for r in d["rounds"]
                ],
                "outages": {
        ...
        if section_mode and not as_json:
    ...
    if what in ("plan-health", "summary", "dashboard", "risk-register", "notify"):
        ...
        for path in sorted(seen):
            ...
            for _pt, _pnum in owners[path]:
                ...
                for tgt, _appr, own, exp, rec, rvw, evi, _sup, _rat, kind, rec_id, recgen, _outcome, _action in _paccs:
                    ...
                    if acceptance_hold(rec, exp, evi, _ptday, _pmatch, _ppath, _ptext, today, kind, tgt, provenance_candidate(_praw, tgt if kind == "run" else None), recgen, _ps.stamp_generation, _outcome) != "cover":
                        continue
                    if rvw < today:
        ...
        # Named results sort as tuples (D00 T04 §6 item 5): the
        # default order compares every field in order, exactly
        # what the old explicit index keys spelled out.
        majors_sorted = sorted(majors)
        criticals_sorted = sorted(criticals)
            ...
            "criticals": [
                {
                    ...
                    "accepted_owner": c.accepted_owner,
                    "accepted_expires": c.accepted_expires,
                    "accepted_review": c.accepted_review,
                    "accepted_rationale": c.accepted_rationale,
                    "accepted_outcome": c.accepted_outcome,
            ...
            "majors": [
                {
                    ...
                    "accepted_owner": m.accepted_owner,
                    "accepted_expires": m.accepted_expires,
                    "accepted_review": m.accepted_review,
                    "accepted_rationale": m.accepted_rationale,
        ...
        if what == "summary":
            ...
            blocked = [c for c in criticals_sorted if not c.accepted_by]
            ...
            for _c in criticals_sorted:
                if _c.overdue:
                    od_by_owner.setdefault(_c.owner or "?", []).append(_c.due)
            ...
            for _b in blocked:
                print(
                    f"    {_b.id}  in {_b.file}  owner {_b.owner or '?'}  due {_b.due or '?'}"
                    + ("  OVERDUE" if _b.overdue else "")
                    + (f"  cause {_b.noncover_cause}; fix: {_b.noncover_fix}" if _b.noncover_cause else "")
                    + (f"  failure {_b.failure_code}" if _b.failure_code else "")
                )
            _debts = [d for d in night_debts(todos, _today_d) if d["open"]]
        ...
        if what == "dashboard":
            ...
            _cu = [c for c in criticals_sorted if not c.accepted_by]
            _mu = [m for m in majors_sorted if not m.accepted_by]
            _co = sum(1 for _c in _cu if _c.overdue)
            _mo = sum(1 for _m in _mu if _m.overdue)
        ...
        if what == "risk-register":
            ...
            if getattr(args, "check", False):
                try:
                    # No newline translation (D00 T01 §34 R1 record
                    # 2): the committed bytes compare raw, so
                    # line-ending drift fails instead of being
                    # translated away at the read.
                    with open(_rpath, encoding="utf-8", newline="") as _rfh:
                        _rcur = _rfh.read()
                except OSError:
                    _rcur = ""
        ...
        for _c in criticals_sorted:
            acct = f"owner {_c.owner or '?'}  due {_c.due or '?'}"
            if _c.overdue:
            ...
            if not _c.owner or not _c.due:
                acct += "  UNACCOUNTABLE"
            if _c.failure_code:
                acct += f"  failure {_c.failure_code}"
            print(f"    {_c.id}  in {_c.file}  {acct}")
        print(
            f"open majors         {len(majors_sorted)} "
            f"({sum(1 for _m in majors_sorted if _m.overdue)} overdue)"
        )
        for _m in majors_sorted:
            acct = f"owner {_m.owner or '?'}  due {_m.due or '?'}"
            if _m.overdue:
...
## 91. Over-long section reference probe
...
> **Verified:** 2026-09-20 | §91 | fixture
> **Review:** round 1 -- Raw findings: docs/reviews/90-health-overlong.md
> **Plan review:** GPT high, filed §9999999999 (run 20260920-D90-T07-S91-gpt)

## 92. Restated-mandatory rationale probe
...
> **Verified:** 2026-09-20 | §92 | fixture
> **Review:** round 1 -- Raw findings: docs/reviews/90-health-restated.md
> **Plan review:** GPT high, filed §93 (run 20260920-D90-T07-S92-gpt)

## 93. Filed-target backlink host
...
Backlink host for D90-T07-S92-PR6 (rule-19 probe).
...
## 112. Stripped generation fails against history
...
> **Verified:** 2026-09-20 | §112 | fixture stripped re-stamp
> **Review:** round 1 -- Raw findings: docs/reviews/90-health-histq112.md
> **Plan review:** GPT high, filed §2 (run 20260920-D90-T07-S112-gpt)

## 113. Kept generation stays silent against history
...
"- [D90-T07-S105-PR0] [minor] quiet row -> accepted\n"
        ...
        for _hq in ("108", "109", "110", "112", "113"):
        ...
        # plus a reason, keeping the valid chain silent under the
        # identity and rationale rules.
        ...
        (rev_dir / "90-health-supersede.md").write_text(
            ...
            "- [D90-T07-S57-PR2] [major] Amended worry -> accepted owner ann due 2099-02-02 supersedes D90-T07-S57-PR1 identity 360709bcdb69 narrowing the scope\n"
        ...
        (rev_dir / "90-health-amend.md").write_text(
            ...
            "- [D90-T07-S87-PR2] [major] Unproven amendment -> accepted owner ann due 2099-02-02 supersedes D90-T07-S87-PR1 fixing the scope\n"
            "- [D90-T07-S87-PR3] [major] Beta worry -> accepted owner ann due 2099-03-03\n"
            "- [D90-T07-S87-PR4] [major] Misidentified amendment -> accepted owner ann due 2099-04-04 supersedes D90-T07-S87-PR3 identity 000000000000 widening the scope\n"
...
"- [D90-T07-S87-PR16] [minor] Filed target two -> accepted\n"
        ...
        # §34 R1 consistency 1: one over-long filed target, named in
        # both the §91 marker and its row, so each side fires once
        # quoting the full run.
        (rev_dir / "90-health-overlong.md").write_text(
            opus_panel
            + "Manifest: sections [D90 T07 §91]; dependents [none]; bytes 100; run 20260920-D90-T07-S91-gpt\n\n"
            "Ledger:\n"
            "- [D90-T07-S91-PR1] [major] Over-long filing -> filed §9999999999 with reason\n"
            "End of ledger\n",
            encoding="utf-8",
        )
        # §34 R1 consistency 2: one head per mandatory-restatement
        # shape, each on its own target. PR5, PR6, and PR7 carry only
        ...
        (rev_dir / "90-health-restated.md").write_text(
            ...
            "Ledger:\n"
            "- [D90-T07-S92-PR1] [major] First worry -> accepted owner ann due 2099-01-01\n"
            "- [D90-T07-S92-PR2] [major] Second worry -> accepted owner ann due 2099-02-02\n"
            "- [D90-T07-S92-PR3] [critical] Third worry -> accepted owner ann due 2099-03-03\n"
            "- [D90-T07-S92-PR4] [major] Fourth worry -> accepted owner ann due 2099-04-04\n"
            "- [D90-T07-S92-PR5] [major] Bare deferral -> deferred owner ann due 2099-05-05 trigger retest supersedes D90-T07-S92-PR1 identity fe08f3b931f1\n"
        ...
        # Tag-object evidence (D00 T04 §6 item 2): the recorded value
        # peels to a different commit, and lineage still proceeds.
        ...
        canned_git[("HEAD", marker_todo.as_posix())] = (
            ...
            .replace(
                "> **Verified:** 2026-09-20 | §112 | fixture stripped re-stamp",
                "> **Verified:** 2026-09-20 | §112 | generation 1 | fixture stripped re-stamp",
                1,
            )
        )
        mbuf = _mio.StringIO()
        ...
        check(
            "an over-long marker reference fires quoting the full run",
            any(
                "TODO-07-marker.md" in ln and "§91 " in ln and "marker names over-long section reference '§9999999999'" in ln
                for ln in marker_out
            ),
            True,
        )
        check(
            "an over-long filed target fires quoting the full run",
            any(
                "TODO-07-marker.md" in ln and "§91 " in ln and "filed target '§9999999999' is over-long" in ln
                for ln in marker_out
            ),
            True,
        )
        check(
            "§91 fires exactly twice (marker plus ledger over-long; no truncated or no-target echo)",
            sum(1 for ln in marker_out if "TODO-07-marker.md" in ln and "§91 " in ln and "FATAL" in ln),
            2,
        )
        check(
            "§93 stays silent (backlink host with no findings)",
            sum(1 for ln in marker_out if "TODO-07-marker.md" in ln and "§93 " in ln and "FATAL" in ln),
            0,
        )
        check(
        ...
        check(
            "stamp-generation-regressed is a FATAL class",
            SEVERITY_MAP.get("stamp-generation-regressed"),
            "fatal",
        )
        check(
            "a stripped generation fails against history",
            any(
                "TODO-07-marker.md" in ln and "§112 " in ln and "stamp generation 0 regresses 1 committed at HEAD" in ln
                for ln in marker_out
            ),
            True,
        )
        check(
            "§112 fires exactly once (the regression)",
            sum(1 for ln in marker_out if "TODO-07-marker.md" in ln and "§112 " in ln and "FATAL" in ln),
            1,
        )
        check(
        ...
        check(
            ...
            any(
                "TODO-07-marker.md" in ln and "§92 " in ln and "d90-t07-s92-pr5 supersedes D90-T07-S92-PR1 with no rationale" in ln
                for ln in marker_out
            ),
            True,
        )
        check(
            "a filed head with only its target fires for no rationale",
            any(
                "TODO-07-marker.md" in ln and "§92 " in ln and "d90-t07-s92-pr6 supersedes D90-T07-S92-PR2 with no rationale" in ln
                for ln in marker_out
            ),
            True,
        )
        check(
            "an accepted-critical head with only owner and due fires for no rationale",
            any(
                "TODO-07-marker.md" in ln and "§92 " in ln and "d90-t07-s92-pr7 supersedes D90-T07-S92-PR3 with no rationale" in ln
                for ln in marker_out
            ),
            True,
        )
        ...
        (clean / "todo" / "90-clean" / "TODO-01-clean.md").write_text(
            ...
            "|   23  |   §23   | Superseding outcomes report in order | -- |  [x]   |\n"
            "\n---\n\n"
            "## 1. Wrong instance\n\n- [x] Did the thing\n- [x] Commit: `\\\"selftest: clean\\\"`\n\n"
            ...
            "## 23. Superseding outcomes report in order\n\n- [x] Did the thing\n- [x] Commit: `\"selftest: clean\"`\n\n"
            "**Test checkpoint:** `true`\n\n> **Verified:** 2026-09-19 | §23 | fixture\n"
            "> **Review:** round 1 -- Raw findings: docs/reviews/90-acc25.md\n"
            "> **Plan review:** GPT high, filed §6 (run 20260919-D90-T01-S23-gpt)\n",
            encoding="utf-8",
        )
        _acc_head = (
            "# Review: fixture\n\n## Opus panel (round 1)\n\n"
...
"Ledger:\n- [D90-T01-S22-PR0] [minor] clean round -> accepted\nEnd of ledger\n"
        ...
        (clean / "docs" / "reviews" / "90-acc25.md").write_text(
            _acc_head
            + "Manifest: sections [D90 T01 §23]; dependents [none]; bytes 100; run 20260919-D90-T01-S23-gpt\n\n"
"Ledger:\n- [D90-T01-S23-PR1] [critical] renewed chain reports in order -> accepted\n- [D90-T01-S23-PR2] [critical] rejected chain voids -> accepted\nEnd of ledger\n"
            f"Risk accepted: D90-T01-S23-PR1; id A1; approver bob; owner bob; date 2026-09-19; expires {_exp_far}; review {_rvw_far}; evidence e23e230000000000000000000000000000000000; rationale old waiver, superseded\n"
            f"Risk accepted: D90-T01-S23-PR1; id A2; approver bob; owner bob; date 2026-09-19; expires {_exp_far}; review {_rvw_far}; evidence e23e230000000000000000000000000000000000; supersedes A1; outcome renewed; rationale renewed chain, order pinned\n"
            f"Risk accepted: D90-T01-S23-PR2; id A3; approver bob; owner bob; date 2026-09-19; expires {_exp_far}; review {_rvw_far}; evidence e23e230000000000000000000000000000000000; rationale old waiver, superseded\n"
            f"Risk accepted: D90-T01-S23-PR2; id A4; approver bob; owner bob; date 2026-09-19; expires {_exp_far}; review {_rvw_far}; evidence e23e230000000000000000000000000000000000; supersedes A3; outcome rejected; rationale rejected chain, voids\n",
            encoding="utf-8",
        )
        ...
        canned_git[("e23e230000000000000000000000000000000000", "docs/reviews/90-acc25.md")] = (clean / "docs" / "reviews" / "90-acc25.md").read_text(
            encoding="utf-8"
        )
        saved_tree, TODO_DIR = TODO_DIR, clean / "todo"
        try:
        ...
        _crit23a = [e for e in _acc_json["criticals"] if e["id"] == "D90-T01-S23-PR1"]
        _crit23b = [e for e in _acc_json["criticals"] if e["id"] == "D90-T01-S23-PR2"]
        check(
            "a rejected record voids even with perfect legs",
            (
                len(_crit23b) == 1
                and _crit23b[0]["accepted_by"] == ""
                and _crit23b[0]["noncover_cause"] == "terminal-outcome"
            ),
            True,
        )
        check(
            "accepted criticals report outcome after an empty failure code",
            (
                len(_crit23a) == 1
                and _crit23a[0]["accepted_by"] == "bob"
                and _crit23a[0]["failure_code"] == ""
                and _crit23a[0]["accepted_outcome"] == "renewed"
            ),
            True,
        )
        ...
        # every field (same-typed neighbors cannot swap), and the
        # field names align with the plan-health JSON keys (the
        # emission covers every field by name).
        check(
            "CriticalFinding fields read in contract order",
            list(CriticalFinding._fields),
            [
                "file",
                "id",
                "owner",
        ...
        check(
            "MajorFinding fields read in contract order",
            list(MajorFinding._fields),
            [
                "file",
                "id",
                "since",
        ...
        check(
            ...
            (
                ...
                _kw_c.escalation,
                _kw_c.accepted_by,
                _kw_c.accepted_owner,
                _kw_c.accepted_expires,
                _kw_c.accepted_review,
                _kw_c.accepted_rationale,
                _kw_c.noncover_cause,
                _kw_c.noncover_fix,
                _kw_c.failure_code,
        ...
        check(
            ...
            (
                ...
                _kw_m.escalation,
                _kw_m.accepted_by,
                _kw_m.accepted_owner,
                _kw_m.accepted_expires,
                _kw_m.accepted_review,
                _kw_m.accepted_rationale,
        ...
        check(
            ...
            set(_crit23a[0].keys()),
            set(CriticalFinding._fields),
        )
        check(
            "plan-health majors JSON keys align with MajorFinding fields",
            set(_maj16[0].keys()) if _maj16 else set(),
            set(MajorFinding._fields),
        )
        _own_src = Path(__file__).read_text(encoding="utf-8")
        check(
            "no positional criticals/majors construction remains",
            re.search(r"(criticals|majors)\.append\(\s*\(", _own_src) is None,
            True,
        )
        check(
            "tag-object evidence proceeds when its commit descends",
            evidence_lineage("e5e6000000000000000000000000000000000000", "ca5e000000000000000000000000000000000000"),
            "ok",
        )
        check(
        ...
        # One open critical, not zero (D00 T04 §6 item 4): the
        # §23 rejected chain voids by terminal-outcome even with
        # perfect legs, so S23-PR2 stays open (pinned by "a
        # rejected record voids even with perfect legs" above).
        check(
        ...
        check(
            "register check fails a missing terminal newline",
            rc_rnoeol,
            1,
        )
        check(
        ...
        check(
            ...
            (
                _blank_register_state("| a | b | live | c | d | e | f | g |\n"),
                _blank_register_state("| a | b | live | c | d | e | f | g |"),
                _blank_register_state("| a | b | live | c | d | e | f | g |\r\n"),
                _blank_register_state("not a row\ntrailing"),
            ),
            (
                "| a | b | | c | d | e | f | g |\n",
                "| a | b | | c | d | e | f | g |",
        ...
        check(
            "a renewed record leaves the legs to decide",
            acceptance_hold("2026-09-19", "2099-01-01", "evi", "", False, "p", "t", "2026-09-20", "finding", "D90-T01-S1-PR0", None, 0, 0, "renewed"),
            "no-match",
        )
        check(
...
+ "; evidence aaa1111000000000000000000000000000000000; supersedes A11; outcome renewed; rationale same-day successor\n",
        ...
        check(
            "XREF section capture refuses over-long runs whole",
            (
                XREF_RE.search(f"§{_long}") is None,
                XREF_RE.search("§3").group("sec"),
                OVERLONG_REF_RE.search(f"§{_long}").group(0),
                OVERLONG_REF_RE.search("§123456789") is None,
            ),
            (True, "3", f"§{_long}", True),
        )
        check(
        ...
        _ok, _why, _ = rp.collect_producer(
            [
                _PY,
                "-c",
                "import sys, time; sys.stdout.write('a\\n' * 100000); sys.stdout.flush(); "
                "time.sleep(0.5); sys.stdout.write('x'); sys.stdout.flush(); time.sleep(30)",
            ],
            b"",
            10,
        )
        check(
            "collector kills a final unterminated line past the line cap",
            (_ok, _why),
            (False, f"output exceeds {rp.OUTPUT_MAX_LINES} lines"),
        )
        ...
        check(
            ...
            (_ok, _why, _info["raw"]),
            (False, f"token exceeds {rp.TOKEN_MAX_CHARS} chars", b"ok\n" + b"x" * 5000 + b"\n"),
        )
        check(
            "collector rejects an unbounded timeout before spawning",
        ...
        check(
            ...
            (
                ...
                # Run uniqueness (D00 T01 §34 R1 integration 1): the
        ...
        _rhuge = _sp.run(
            _rtimeout_argv + ["--timeout", "99999999", "--", sys.executable, "-c", "print('hi')"],
            capture_output=True,
            text=True,
            timeout=30,
        )
        check(
            "review-run rejects infinite and over-max timeouts",
            (
                _rinf.returncode,
                _rhuge.returncode,
                "positive number of seconds up to" in _rinf.stderr,
                "positive number of seconds up to" in _rhuge.stderr,
            ),
            (2, 2, True, True),
        )
        ...
        _rrace_argv = [
            ...
            sys.executable,
            "-c",
            "import time; print('**adversarial: approve**\\n\\n**consistency: approve**\\n\\n**integration: approve**\\n\\n**record: approve**'); time.sleep(2)",
        ]
        _rp1 = _sp.Popen(_rrace_argv, stdout=_sp.PIPE, stderr=_sp.PIPE, text=True)
        _rp2 = _sp.Popen(_rrace_argv, stdout=_sp.PIPE, stderr=_sp.PIPE, text=True)
        _rp1.communicate(timeout=60)
        _rp2.communicate(timeout=60)
        _rrace_lines = (_rrace / "ledger.jsonl").read_text(encoding="utf-8").splitlines()
        check(
        ...
        (rev_dir / "90-health-overlong.md").unlink()
        (rev_dir / "90-health-restated.md").unlink()
        marker_todo.unlink()
        panel_todo.unlink()
        for extra in (
            "90-panel-nopanel.md",
...
""",
        ...
        matrix_rows = re.findall(
            r"^\|\s*`(renewed|remediated|rejected|closed)`\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|$", readme, re.M
        )
        check(
            "README outcome matrix matches OUTCOME_MATRIX row-for-row, in order",
            [(o, c, b) for o, c, b in matrix_rows],
            [(o, c, b) for o, (c, b) in OUTCOME_MATRIX.items()],
        )
        check(
        ...
        # --- query telemetry (D00 T01 §39) -----------------------------------
        import io as _tio
        import contextlib as _tctx
...
- `adversarial` approve
- `consistency` needs-attention: (R1-F1) wobble.
Telemetry: round 1; model gpt-5.6-sol; effort medium; duration 84s; outcome needs-attention; tokens 16,561

| ID | Disposition | Evidence |
| R1-F1 | live | still reporting |
...
- `adversarial` approve
- `record` approve
Telemetry: round 2; model opus; effort medium; duration unknown; outcome approve; tokens unknown

| ID | Disposition | Evidence |
| R1-F1 | fixed | bbb2222 |
"""
        _TEL_BAD = """Telemetry: round 9; model x; effort y; duration 1s; outcome approve; tokens 1
## GPT panel
- `adversarial` approve
Telemetry: round 5; model gpt-5.6-sol; effort medium; duration 3s; outcome approve; tokens 100
Telemetry: round 5; model gpt-5.6-sol; effort medium; duration soon; outcome approve; tokens 12x
Telemetry: round 1; model gpt-5.6-sol; effort medium; duration 4s; outcome approve; tokens 200
Sol outage: none
Sol outage: No rounds ran here
Sol outage: CLI missing before round 1
Opus outage model failure at sign-off
"""
        _tp = telemetry_parse(_TEL_MD)
        check("telemetry parser finds both rounds", [(r["n"], r["family"]) for r in _tp["rounds"]], [(1, "GPT"), (2, "Opus")])
        _t1 = _tp["rounds"][0]["telemetry"]
        check("telemetry strips comma tokens", _t1["tokens"], 16561)
        check("telemetry parses duration seconds", _t1["duration"], 84)
        check("telemetry keeps model and outcome", (_t1["model"], _t1["outcome"]), ("gpt-5.6-sol", "needs-attention"))
        _t2 = _tp["rounds"][1]["telemetry"]
        check("telemetry unknowns parse to None", (_t2["duration"], _t2["tokens"]), (None, None))
        check(
            "telemetry verdicts allow trailing prose like the validator",
            _tp["rounds"][0]["verdicts"],
            [("adversarial", "approve"), ("consistency", "needs-attention")],
        )
        check("telemetry disposition rows carry round context", _tp["disps"], [("R1-F1", "live", "still reporting", 1), ("R1-F1", "fixed", "bbb2222", 2)])
        check("telemetry runs extract shaped IDs", _tp["runs"], ["20260920-D90-T09-S1-gpt"])
        _tb = telemetry_parse(_TEL_BAD)
...
domain: 90-selftest
status: active
title: "TODO-09 -- Telemetry Ref"
track: Z1
---

# TODO-09 -- Telemetry Ref
...
- [x] Did the thing
- [x] Commit: `"selftest: telemetry"`

**Test checkpoint:** `true`

> **Verified:** 2026-09-20 | §1 | fixture
> **Review:** round 1 -- Raw findings: docs/reviews/90-tel-s1.md
...
""",
        ...
        with _tctx.redirect_stdout(_sbuf), _tctx.redirect_stderr(_tio.StringIO()):
            _scode = cmd_query(
                argparse.Namespace(what="telemetry", target=str(_rev / "90-tel-s1.md"))
            )
        _slines = _sbuf.getvalue().splitlines()
        check("query telemetry exits 0 on a file target", _scode, 0)
        check(
            "query telemetry section shows round detail",
            any("R1 (GPT)" in ln and "16561" in ln for ln in _slines),
            True,
        )
        check(
        ...
        _rbuf = _tio.StringIO()
        with _tctx.redirect_stdout(_rbuf), _tctx.redirect_stderr(_tio.StringIO()):
            _rcode = cmd_query(argparse.Namespace(what="telemetry", target="D90 T09 §1"))
        check("query telemetry exits 0 on a section ref", _rcode, 0)
        check(
            "query telemetry ref resolves to the findings file",
            any("90-tel-s1.md" in ln for ln in _rbuf.getvalue().splitlines()),
            True,
        )
        _jbuf = _tio.StringIO()
        with _tctx.redirect_stdout(_jbuf), _tctx.redirect_stderr(_tio.StringIO()):
            _jcode = cmd_query(
                argparse.Namespace(what="telemetry", target="D90 T09 §1", json=True)
            )
        ...
        with _tctx.redirect_stdout(_bbuf), _tctx.redirect_stderr(_tio.StringIO()):
            _bcode = cmd_query(
                argparse.Namespace(what="telemetry", target=str(_rev / "90-tel-bad.md"))
            )
        _blines = _bbuf.getvalue().splitlines()
        check("query telemetry exits 0 on malformed lines", _bcode, 0)
        check(
            "query telemetry reports mismatches and malformed counts",
            any("mismatches: 1" in ln for ln in _blines)
            and any("malformed telemetry lines skipped: 3" in ln for ln in _blines),
            True,
        )
        _ubuf = _tio.StringIO()
        with _tctx.redirect_stdout(_ubuf), _tctx.redirect_stderr(_tio.StringIO()):
            _ucode = cmd_query(argparse.Namespace(what="telemetry", target="D90 T09 §9"))
        check("query telemetry exits 1 on an unknown section", _ucode, 1)
        _nbuf = _tio.StringIO()
...
def main() -> int:
    ...
    q.add_argument("target", nargs="?", default=None, help="run: run ID to inspect; telemetry: section ref or findings path for the per-section view")
