...
- Command outcome: a bound menu item's `Click` handler must be the one its id names (`MenuFileNewTab` runs `OnFileNewTab`), so a chord re-wired to another command fails the manifest; a covering test must call an `Assert.` after its press, so a press that proves only that a key went out covers nothing. Fixtures: `BindingManifestTests.PlantedHandlerSwapFails`, `CoveringTestWithoutAnOutcomeAssertionFails`.
