...
Soak runs locally, usually as an unattended bedtime run: five extra repetitions of the flake-prone suites (UI and Protocol) after the governed legs (the solution run is Run A), all with `trx` loggers; the CI `soak` workflow repeats the same shape nightly. The command below shows the local shape.

`dotnet test tests/UI --logger trx` plus `dotnet test tests/Protocol --logger trx`, each x5 (the governed shape lives in `docs/testing.md`).
...
| `UI.SnapshotTests.RestoreDontSaveReplacesBuffer` (`snapshot-dontsave-dialog-null`) | xUnit `Assert.NotNull` on the re-shown `SnapshotsDialog` in `WaitForDialog` after answering the save prompt Don't-save: the dialog never reappeared. Proof: commit `c62ff14` red in the Snapshot fixture (7/8, this test failed), green solo on the same commit (1/1; counts quoted in `docs/reviews/00-workspace/D00-T02-s11.md`). Likely cause: dialog-flow timing under machine load (same family as the 2026-09-20 quarantine batch; fails on the clean tree, so not §11's seeding). | 2026-09-20 | D01 T01 §16 | 2026-09-20 | 2026-09-27 |
| `UI.SnapshotTests.RestoreCancelKeepsBuffer` (`snapshot-cancel-dialog-null`) | xUnit `Assert.NotNull` on the re-shown `SnapshotsDialog` in `WaitForDialog` after answering the save prompt Cancel. Proof: commit `c62ff14` red in the Snapshot fixture (6/8), green in the repeat fixture (7/8; counts quoted in `docs/reviews/00-workspace/D00-T02-s11.md`). Likely cause: dialog-flow timing under machine load. | 2026-09-20 | D01 T01 §16 | 2026-09-20 | 2026-09-27 |
| `UI.SnapshotTests.RestoreSaveWritesThenRestores` (`snapshot-save-dialog-null`) | xUnit `Assert.NotNull` on the re-shown `SnapshotsDialog` in `WaitForDialog` after answering the save prompt Save. Proof: commit `c62ff14` red solo, green solo on the same commit (1/1 each; counts quoted in `docs/reviews/00-workspace/D00-T02-s11.md`). Likely cause: dialog-flow timing under machine load. | 2026-09-20 | D01 T01 §16 | 2026-09-20 | 2026-09-27 |
| `UI.SnapshotTests.RestoreSaveFailureAbortsWithWorkPreserved` (`snapshot-savefailure-dialog-null`) | xUnit `Assert.NotNull` on the re-shown `SnapshotsDialog` in `WaitForDialog` after answering the save prompt Save on a read-only file. Proof: commit `c62ff14` red in the Snapshot fixture (5/8), green in the repeat fixture (6/8; counts quoted in `docs/reviews/00-workspace/D00-T02-s11.md`). Likely cause: dialog-flow timing under machine load. | 2026-09-20 | D01 T01 §16 | 2026-09-20 | 2026-09-27 |
| `UI.SettingsStoreNoticeTests.CorruptSettingsShowsNoticeAndRestoresDefaults` (`settings-corrupt-dialog-null`) | xUnit `Assert.NotNull` on `CorruptSettingsDialog` in `DismissDialog` at startup: the notice dialog never appeared. Proof: Run A attempt 2 (2026-09-20) red in the full suite, green solo on the same commit; counts quoted in `docs/reviews/00-workspace/D00-T02-s11.md`. Likely cause: dialog-flow timing under machine load. | 2026-09-20 | D01 T02 §2 | 2026-09-20 | 2026-09-27 |
