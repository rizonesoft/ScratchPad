...
The morning report lands at `build/nightly/morning-YYYY-MM-DD.md`: date, HEAD, trigger, a per-leg table (passed, failed, skipped-with-reason, gate verdict, log path), every failure with its filed finding ref (triage appends refs after filing each failure in the owning file, before the next section starts), and uncollected debt as information with its cause. A Night debt section carries one entry per debt open at run start (collected counts with log, or uncollected with cause), and red Interactive collections stage finding stubs under Filings for triage to file. The current report is the worked example of the format.
