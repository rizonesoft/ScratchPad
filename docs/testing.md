...
Foreground proof rides alongside the default run: `Bin/ForegroundLog/Debug/ForegroundLog.exe <seconds> <logpath>` polls the foreground window and exits 1 naming the run red if ScratchPad ever held it. Green means the default suite passed with zero failures AND the log flagged zero foreground holds; a passing suite with a flagged log is a gate failure (some test activated a window). The fenced set is green when it passes visibly. The input audit behind the split lives in `docs/ui-input-audit.md`.
