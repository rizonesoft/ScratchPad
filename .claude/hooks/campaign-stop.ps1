# Stop hook for the Claude process-plan campaign.
# Blocks the campaign session's end of turn while its run is open, and
# lets go when the run is finished, parked, out of runnable work, or
# stalled. Only the session named in the guard file is ever blocked.
# Fail open: a bad payload, a missing guard, or a thrown error allows
# the stop.
#
...
try {
    ...
    try {
        ...
        $ready = & python3 scripts/todo-graph.py query ready 2>$null
    ...
    $message = "Campaign run is still open ($relative). Do not end the turn. Finish the open section's checklist, run the review panel and stamp it, then the next section, then the next phase. A commit, a green suite, or a status report is not a stop."
