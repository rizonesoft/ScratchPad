...
python3 scripts/todo-graph.py query findings | head -20
```
...
A new section carries everything the format requires from birth: context paragraph, micro-step checklist with a `Commit:` item, `Test checkpoint:`, Fidelity/Job/Treatment/Chrome when it builds a surface, `Needs:` when it needs a Windows host, and its Implementation Order row with a real `Depends On`. A section filed as a one-line stub is a plan defect, not a head start.
