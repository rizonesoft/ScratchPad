...
# Every matching line, not only the first (section 32 R2-C1): a value
# summed across assemblies or files names each contributing line.
$locateAll = { param($pattern, $regex) $hits = @(); foreach ($f in @(Get-ChildItem $RunDir -Filter $pattern -Recurse -File -ErrorAction SilentlyContinue | Sort-Object FullName)) { $n = 0; foreach ($ln in [System.IO.File]::ReadLines($f.FullName)) { $n++; if ($ln -match $regex) { $hits += "$($f.FullName.Substring($RunDir.TrimEnd([char]92, '/').Length).TrimStart([char]92, '/')):$n" } } }; $hits -join ', ' }
...
$result = [pscustomobject]@{
  ...
  provenance = [pscustomobject]@{
    ...
    'legs.run-a.counts' = [pscustomobject]@{ source = $(if ($transcriptLegs['run-a']) { "transcript $(& $artifacts '*-default.log')" } else { "Run A row in $($reportFile[0].Name)" }); method = $(if ($transcriptLegs['run-a']) { 'transcript-parse' } else { 'report-row' }); locator = $(if ($transcriptLegs['run-a']) { & $locateAll '*-default.log' '(Passed|Failed)!' } else { & $locate $reportFile[0].Name 'Run A \(default\)' }); confidence = 'derived' }
    'legs.run-b.counts' = [pscustomobject]@{ source = $(if ($transcriptLegs['run-b']) { "transcript $(& $artifacts '*-primary.log')" } else { "Run B row in $($reportFile[0].Name)" }); method = $(if ($transcriptLegs['run-b']) { 'transcript-parse' } else { 'report-row' }); locator = $(if ($transcriptLegs['run-b']) { & $locateAll '*-primary.log' '(Passed|Failed)!' } else { & $locate $reportFile[0].Name 'Run B \(primary\)' }); confidence = 'derived' }
    ...
    'timings.run-a' = [pscustomobject]@{ source = "test-seconds in $(& $artifacts '*-default*.log')"; method = 'transcript-parse'; locator = (& $locate '*-default*.log' 'test-seconds: [0-9]+'); confidence = 'derived' }
    'timings.interactive' = [pscustomobject]@{ source = "test-seconds in $(& $artifacts '*-full*.log')"; method = 'transcript-parse'; locator = (& $locateAll '*-full*.log' '^(Start|End) time: [0-9]{14}'); confidence = 'derived' }
    'legs.run-a.gate' = [pscustomobject]@{ source = "Run A gate cell in $($reportFile[0].Name)"; method = 'report-row'; locator = (& $locate $reportFile[0].Name 'Run A \(default\)'); confidence = $(if ($legA.ran -and ($null -eq $legA.gate)) { 'unknown' } else { 'derived' }) }
    'legs.run-b.gate' = [pscustomobject]@{ source = "Run B gate cell in $($reportFile[0].Name)"; method = 'report-row'; locator = (& $locate $reportFile[0].Name 'Run B \(primary\)'); confidence = $(if ($legB.ran -and ($null -eq $legB.gate)) { 'unknown' } else { 'derived' }) }
    'timings.run-b' = [pscustomobject]@{ source = "test-seconds in $(& $artifacts '*-primary*.log')"; method = 'transcript-parse'; locator = (& $locate '*-primary*.log' 'test-seconds: [0-9]+'); confidence = 'derived' }
    'incidents' = [pscustomobject]@{ source = $(if ($incidentsDerived) { "trx failures in $(& $artifacts '*.trx')" } else { "INC lines in $($reportFile[0].Name)" }); method = $(if ($incidentsDerived) { 'trx-parse' } else { 'report-line' }); locator = $(if ($incidentsDerived) { & $locateAll '*.trx' 'outcome="Failed"' } else { & $locateAll $reportFile[0].Name 'INC-[0-9a-f]{8}' }); confidence = $(if ($incidentsDerived) { 'derived' } else { 'native' }) }
    'soak' = [pscustomobject]@{ source = "soak trx $(& $artifacts '*-soak-*.trx')"; method = 'trx-parse'; locator = (& $locateAll '*-soak-*.trx' '<(Counters|UnitTestResult)'); confidence = 'derived' }
