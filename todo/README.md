...
| `quorum-same-family` | FATAL | A clean review in the same family as its quorum reference (through 2026-09-22 the sign-off panel, the last panel section; after it the writer family, Claude): quorum is one independent second-family pass, so a GPT review under a GPT sign-off panel met no quorum and owes `retry-owed` like any same-family survivor. On stamps after 2026-09-22 the reference is the writer family, Claude, not the sign-off family (D00 T04 §23): a claude plan-review run owes `retry-owed` and a GPT run is the independent pass. The run's family reads from the legacy `-gpt`/`-opus` suffix or the minted `-codex`/`-claude` suffix with its optional `-c<hex>` and `-rN`. Outage markers, `retry-owed` markers, partial markers (rule 21's family-relative composition owns their debt), and family-opaque markers stay silent under their own rules. Stamps on or before 2026-09-20 predate the rule and are grandfathered. |
...
| `panel-slots` | FATAL | A panel-slot wiring break: `.conclave/panel.toml` is missing while skills name slots, or it fails to parse, names a model outside `gpt-6-sol`, `gpt-5.6-terra`, `claude-opus-5-5` or an effort outside `medium`, `high`, `xhigh`, misses a governed slot, carries an ungoverned one, or breaks the failover effort-parity rule; or a review-skill command names a `--slot` the TOML does not carry, or a governed slot runs through no skill command, or pins a model or effort literally inside a fenced command block. Roots without the file skip: fixture trees predate the wiring. |
