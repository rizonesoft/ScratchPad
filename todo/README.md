...
| `panel-sol-outage-missing` | FATAL | An Opus-only findings file (Opus panel sections, zero GPT panel sections) attached to a stamp dated after 2026-09-19 that carries no `Sol outage: <what failed>` line naming the failure. Skipped Sol rounds cannot pass as planned-mixed records; s25 (stamped same-day pre-rule, all-Opus by runner practice) is the named grandfather the birthday boundary exempts. The note is a line containing `Sol outage:` plus failure text (no CLI, auth failure, model error, or timeout); bare mentions, `No`-prefixed denials, nothing-valued notes (none, n/a, nothing, never, not applicable), and empty notes fire. Fenced code blocks are stripped before the scan. |
