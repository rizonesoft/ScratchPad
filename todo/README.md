...
| `owner-login-unmapped` | WARN | An owner named by a verified marker, ledger row, or acceptance line with no login in the mapping. The poster degrades to an unassigned issue naming the gap, so the warning rats until mapped; a red build cannot invent a login. |
