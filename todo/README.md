...
| `quorum-same-family` | FATAL | A clean review whose sign-off panel (last panel section) is the same family: quorum is one independent second-family pass, so a GPT review under a GPT sign-off panel met no quorum and owes `retry-owed` like any same-family survivor. Outage markers, `retry-owed` markers, partial markers (rule 21's family-relative composition owns their debt), and family-opaque markers stay silent under their own rules. Stamps on or before 2026-09-20 predate the rule and are grandfathered. |
