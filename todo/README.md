...
> **Plan review:** GPT outage then Opus auth failure, outage: both rungs (owner ann, due 2026-09-25) class auth attempts 2 event 2026-09-19
...
- `Plan review:` -- the second-family round's completion marker: which family ran it and the filings it produced, `no findings`, `outage: <rung> (owner <name>, due <YYYY-MM-DD>) class <class> attempts <n> event <YYYY-MM-DD>` when both runners failed, `retry-owed (owner <name>, due <YYYY-MM-DD>) class <class> attempts <n>` on a same-family fallback run, or `partial: <rung>` when one rung failed and the other's findings stand (filings beside `partial:` are the survivor's; the failed rung is `gpt rung` or `opus rung`: the survivor owes `retry-owed (owner, due)` exactly when it is the same family as the quorum reference (through 2026-09-22 the sign-off panel: normally the fallback rung; flipped under a GPT sign-off panel on stamps through 2026-09-22; on stamps after it the reference is the writer family, Claude, so the claude-rung survivor always owes it, D00 T04 §23), and a cross-family survivor carries no retry and no accountability fields but carries `class`/`attempts` like every degraded line; quorum is one independent second-family pass, which the cross-family review meets and the same-family review owes). Lineage: every marker over a review record carries `(run <YYYYMMDD-DNN-TNN-SN-family[-rN]>)` minted by the `run-id` subcommand, the run's date prefix is its timestamp; within one date base the bare base is run 1 and `-rN` is run N for N >= 2 (numbering restarts per day, the date keeps runs distinct) (`-r1` is run 1's synonym and reads as the base in every comparison, `-r0` is outside the shape, and non-ASCII or longer-than-9-digit suffixes fail the mint with a naming diagnostic); genesis is a singleton marker (run, no supersedes), and a rerun marker chains with `supersedes <prior-run>` or with `follows-outage` when the immediately preceding marker is an outage marker; runs never repeat within a section and the last run is one the manifest carries. A rerun opens a new `Plan review` record (rows number past the file max across records). Grammar: the last marker line governs; `no findings` never sits beside filings; `outage:` never sits beside filings, `no findings`, `retry-owed`, or `partial:`; `partial:` composes with `retry-owed` exactly when the survivor is the same family as the quorum reference (through 2026-09-22 the sign-off panel: normally the fallback, flipped under a GPT sign-off panel; after it the writer family, Claude, so always the claude-rung survivor, D00 T04 §23). Every outage, retry-owed, or partial line in the chain carries `class <class>` (failure class, open vocabulary) and `attempts <n>` (positive ASCII attempt count of 1-9 digits, whole token; validator-enforced; per-line scope counting the attempts behind that line's stated outcome, initial attempt included, reruns reset with no cross-line arithmetic; composite lines carry one `attempts` count per outcome in outcome order, the class riding line-level or per outcome), superseded outage lines included, so recurring failures are diagnosable from the record; every outage line in the chain additionally names `event <YYYY-MM-DD>` (the calendar day the outage happened, a real date; validator-enforced, prose-outage excluded), so two outages of one rung on different days under one stamp key differently and a waiver for one never covers the other (same-day same-rung pairs still share a key); evidence rides the findings-file outage note, which opens with `Outage note: <rung> <event-day>` keyed to the marker (unlinked markers, orphan or duplicated notes, and malformed keys are FATAL past the 2026-09-20 grandfathering), and retry linkage is the `follows-outage` chain. Counts read in accepted findings (triaged ledger rows) versus implementation items (checklist lines); a merge states both numbers. Required on stamps dated after 2026-09-18; earlier stamps predate the rule. Marker lines amend append-only (rule 28): a line edited or deleted against history fails; chains grow by appending new lines.
- `Reopened:` -- `<YYYY-MM-DD> | generation <n> | <finding ref> | <reason>`, naming the audit locus whose finding voids the stamp. The generation is previous-plus-one of the stamp generation; the re-stamp persists it on the `Verified:` evidence (`generation <n> |`), so no pre-reopen acceptance revives when the line lifts. A reopened section reads as unverified everywhere downstream: its row must be `[ ]` and its stamped dependents park until it re-stamps.
...
- `Duration:` -- optional minutes or instant range from `Started:` to stamp (implement, review, and stamp): either integer minutes (`7`, `45m`) or `<start> to <end>` Zulu instants (`2026-09-18T14:54:33Z to 2026-09-18T16:35:19Z`). The range end orders clearance: when both reviews carry ends the target must complete strictly after the finding's review, else day stamps rule and same-day fails closed. Ranges are needed exactly when clearance must order inside one day (the ordering, recency, and attestation legs read instant ends; minute forms and dateless stamps cannot order same-day pairs). The normal loop qualifies without a mandate: write `Started:` when implementation starts, range it to stamp completion, and rule 26 (`duration-range-uncheckable`) ties both ends to those dates on post-cutoff stamps. Minutes floor (`// 60`, so a sub-minute span reads 0 minutes); progress JSON carries exact `duration_seconds` beside them, so no real work reads as zero.
...
- `Night-owed:` -- one line per night debt: `**Night-owed:** <DEBT-ID> (<count> <filter>, collector <collector>, owed <YYYY-MM-DD>[, ...])`, where the id is `<domain>-T<todo>-S<section>-N<n>` (for example `D00-T02-S8-N1`), the filter is a bare `Category` value or a full `--filter` expression, and prose may follow the parens (the parser reads the id plus the paren fields and ignores the rest). New debt always carries `owed`; debt predating the format ages from its section's stamp date. Debt owed on or after 2026-09-26 also carries `digest <hex>`, the owed tests' digest (`powershell -File tools/Get-DebtDigest.ps1 -Filter "<filter>"` prints it), and closes only on a collection whose `digest` matches (D00 T02 §42); earlier debt closes on counts. Optional chunks (D00 T02 §50): `tests <path>` names the owed inventory file (the test names as `--list-tests` prints them, one per line; a later change to it never changes what closes the debt), `candidate <sha>` binds closure to a collection on that commit or a descendant, `tz <zone>` and `schedule <version>` record the schedule identity the due rests on (an owed timestamp's own offset stands in for `tz`), and an inline `due <date>` past the default window needs `by <approver>` and a `reason` and obeys the same 21-day cap as `Night-extend`. From 2026-09-27 the capture is required: an owed line names its `candidate`, `schedule` (the schedule history's version, its From date), `tz` (or an offset on its timestamp), and `tests` (its inventory), or no collection can close it; debt owed before that day is not migrated and keeps closing on its digest (or, before 2026-09-26, its counts); the trigger comes from `docs/nightly-schedule-history.md` for the owed day.
- `Night-collected:` -- one line per collection proving a debt closed: `**Night-collected:** <YYYY-MM-DD> <DEBT-ID> (<passed> passed, <failed> failed, <skipped> skipped; log <path>[; digest <hex>])`, appended to the owning section by the collector. It may carry `candidate <sha>` and `run <id>` (§50). Every lifecycle line may carry `at <HH:MM>` (its effective local time) and `event <id>`: records replay by date then effective time, never by wording, and records of one kind that tie on both, or share an event id, but differ are contradictory and none of them takes effect until resolved. From 2026-09-27 a collection without `candidate`, `run`, `at`, and `event`, and any other lifecycle record without `at` and `event`, warns and takes no effect; the collector writes them. A debt id is open while it has a `Night-owed:` line and no `Night-collected:` line naming it; green collection of the debt's exact filter with matching census appends and closes (skipped counts are quarantine-declared only; capability or other skips hold the debt open), a superset green run closes fully executed FQN-attributable subsets with their own counts and stages the rest for triage, red collection stages findings and stays open for the next window.

### Completion-first: ship with night debt

A section ships its focus-free proofs, records Interactive skips as `Night-owed` debt, and flips the same session; review and stamp never wait for quiet time. The `\ScratchPad\Nightly UI` run's Interactive leg (`tools/nightly.ps1`, D00 T02 §9) is the collector: green collection appends `Night-collected:` and closes the debt, red collection stages finding stubs in the morning report and the debt stays open. The only true flip blockers are red focus-free or Primary proofs, unmet Depends, a missing baseline artifact, an unreachable host, and both review families down; open night debt is information, never a blocker, and quiet time never parks work.
...
python3 scripts/todo-graph.py query telemetry      # panel telemetry: tree round/token/family totals, GPT-outage coverage (legacy Sol-worded included), per-section summary (target: section ref or findings path; --json emits schema telemetry/1)
...
| `marker-history` | FATAL | A `Plan review:` marker line edited or deleted against the committed history. Marker chains append, never rewrite, so coverage cannot silently move. |
...
| `stamp-no-opus-panel` | FATAL | A stamp dated after 2026-09-17 whose findings lack a governing panel section with all four lens verdicts (through 2026-09-22 the sign-off panel: `Opus panel` through the label cutover, `Claude panel` on no stamp since both cutovers share 2026-09-22; after it the last `GPT panel` section, or a `Claude panel` cross-fill with a `GPT outage` line naming sol and terra). It reads as reviewed evidence while verifying nothing; stamps on or before 2026-09-17 predate the rule and are grandfathered. Stamps after 2026-09-23 read the six-slot panel (D00 T04 §25), with families taken from `.conclave/panel.toml`, never from code: the governing last section is the primary family's panel (the `signoff` slot's family, `GPT panel` today) with all four verdicts, or the fallback family's panel (`Grok panel` today) plus a `<primary label> outage:` line; any writer-family section (`[panel] writer`, Claude today, legacy `Opus panel` included) fires. Stamps dated 2026-09-23 only are GPT-governed under D00 T04 §23: a `GPT panel` last section needs no note, and a `Claude panel` last section needs a `GPT outage:` line naming both sol and terra; after 2026-09-23 no Claude section is valid. The Claude-governs reading below applies to stamps through 2026-09-22. The `Review:` line must carry `Raw findings: <path>`; the panel section is a level-2+ heading starting with the era's words (last one wins in multi-round files); each verdict sits on its own `` `lens` verdict `` line opening (after up to 3 spaces) with a Markdown marker (`*`, backtick, `>`, `-`; mid-line mentions never count); fenced code blocks are stripped before the scan, quoted headings are not structure, quoted fences count as fences (closes match the opener's quote depth; an ended quote ends its fence, and a blank line ends the quote), a backtick in a backtick-fence info string is a paragraph, and an unbalanced fence fails naming its opener line. Through 2026-09-22, a `GPT panel` section (the fallback when the sign-off panel is unreachable) satisfies the rule when it carries all four lens verdicts in the same shape plus a line with the era's outage words (`Opus outage` through 2026-09-22, `Claude outage` after); the last panel section of either family governs. Planned `GPT panel` early rounds under a sign-off (the §35 mixed sequence) need no outage note; only a `GPT panel` last section does. Record words cut over on 2026-09-22 (D00 T04 §14): D00 T01 §55, stamped cutover-day with legacy words, is the named same-day grandfather. Post-cutover records carrying any legacy `Opus panel` section fire even when a valid new-era section governs; pre-cutover records carrying any new-word `Claude panel` section fire even when a valid old-era section governs. |
| `panel-sol-outage-missing` | FATAL | A sign-off-only findings file (sign-off panel sections, zero GPT panel sections) attached to a stamp dated after 2026-09-19 that carries no era outage line (`Sol outage: <what failed>` on stamps through 2026-09-22, `GPT outage: <what failed>` after) naming the failure. Skipped Sol rounds cannot pass as planned-mixed records; s25 (stamped same-day pre-rule, all-Opus by runner practice) is the named grandfather the birthday boundary exempts, and D00 T01 §55 (stamped 2026-09-22, legacy words on cutover day) is the named grandfather the label cutover exempts. The note is a line containing the era's words plus failure text (no CLI, auth failure, model error, or timeout); bare mentions, `No`-prefixed denials, nothing-valued notes (none, n/a, nothing, never, not applicable), and empty notes fire. Fenced code blocks are stripped before the scan. After 2026-09-22 a Claude-only record fires as `stamp-no-opus-panel` instead (Claude governs only as the double-GPT-outage fill, D00 T04 §23), so this class stays silent for it. |
| `outage-note-unlinked` | FATAL | An outage marker whose (rung, event-day) key resolves to no `Outage note: <rung> <YYYY-MM-DD>` line in the section's findings file, a note no marker keys, or a duplicated or malformed note key. Only the note carries the failure evidence, so an unlinked marker reads as recorded while supporting nothing. Chain-wide across superseded outage lines; matching is file-wide across sections sharing one findings file (pre-cutoff sections pool their keys but never fire); fenced code blocks are stripped before the scan; stamps on or before 2026-09-20 predate the rule and are grandfathered. |
| `quorum-same-family` | FATAL | A clean review in the same family as its quorum reference (through 2026-09-22 the sign-off panel, the last panel section; after it the writer family, Claude): quorum is one independent second-family pass, so a GPT review under a GPT sign-off panel met no quorum and owes `retry-owed` like any same-family survivor. On stamps after 2026-09-22 the reference is the writer family, not the sign-off family (D00 T04 §23): a writer-family plan-review run owes `retry-owed` on stamps dated 2026-09-23, and fires outright after it (D00 T04 §25, writer read from `[panel] writer`), while any other family's run is the independent pass. The run's family reads from the legacy `-gpt`/`-opus` suffix or the minted `-codex`/`-claude`/`-grok` suffix with its optional `-c<hex>` and `-rN`. Outage markers, `retry-owed` markers, partial markers (rule 21's family-relative composition owns their debt), and family-opaque markers stay silent under their own rules. Stamps on or before 2026-09-20 predate the rule and are grandfathered. |
| `owner-logins-invalid` | FATAL | `.github/owner-logins.json` is missing its shape: not a JSON string-to-string object, or any login outside GitHub login rules (alphanumerics plus single hyphens, max 39, never leading or trailing hyphen). The poster assigns off this file, so a malformed one breaks delivery. Roots without the file skip: fixture trees carry no GitHub identity. |
| `owner-login-unmapped` | WARN | An owner named by a verified marker, ledger row, or acceptance line with no login in the mapping. The poster degrades to an unassigned issue naming the gap, so the warning waits until mapped; a red build cannot invent a login. |
...
| `stamp-no-plan-review` | FATAL | A stamp dated after 2026-09-18 that carries no `Plan review:` completion marker, or whose marker breaks the grammar. The second-family round runs after panel-close and its marker rides the stamp commit, naming the filings, `no findings`, `outage: <rung> (owner, due) class <class> attempts <n> event <date>`, `retry-owed (owner, due) class <class> attempts <n>`, or `partial: <rung>` (one rung failed, the other's findings stand); stamps on or before 2026-09-18 predate the rule and are grandfathered, so mechanical enforcement begins 2026-09-19 and landing-day stamps carry the marker voluntarily. The last marker line governs; `no findings` beside filings, and filings, `no findings`, `retry-owed`, or `partial:` beside `outage:`, are FATAL. A `partial:` names its failed rung (`gpt rung` always; `opus rung` on stamps through 2026-09-22, `claude rung` on stamps dated 2026-09-23, the TOML's primary and fallback rungs after it, `grok rung` today, where no survivor owes `retry-owed`, D00 T04 §25); through 2026-09-23 a fallback survivor without `retry-owed`, a primary survivor with `retry-owed` or accountability fields, and an unknown rung are FATAL; after it any survivor with `retry-owed` or accountability fields and an unknown rung are FATAL, since both rungs are independent of the writer. An outage, retry-owed, or partial line without `class <class>` or a positive `attempts <n>` (ASCII digits of length 1-9, whole token: `2x`, `²`, and longer runs fire) is FATAL, superseded lines included; a composite (retry-owed plus partial) line without exactly two well-formed counts in outcome order fires the same way. An outage line without a real `event <YYYY-MM-DD>` outage-event date is FATAL, superseded lines included, prose-outage excluded. Every named filing must resolve, and every ledger `filed` row's target must appear in that section's marker, checked per marker with no cross-section dedup (`outage:` markers skip both). |
| `plan-review-malformed` | FATAL | A post-cutoff plan-review record the query cannot parse: a `Plan review` section without its `Manifest:` line or its `Ledger:`/`End of ledger` block, a non-blank line inside the block outside the row shape, a content-illegal row (a `deferred` row without owner, due, and trigger; a `duplicate` row naming no canonical finding; a `filed` row naming no target; an `accepted` critical or major without owner and due, the deferred triple satisfying accountability through its due date), or a run-less manifest on a chain that carries runs (run-less chains stay exempt: outage-only records name no run by design). Rows are only rows inside the block; `- [` lines outside it are prose. Pre-cutoff records predate the shapes and are grandfathered, except the retired `date` spelling of the deferred triple (removed 2026-09-21: any `date`-spelled deferred row fires regardless of stamp date). |
...
| `stamp-reopened` | FATAL | A `> **Reopened:**` line outside `<YYYY-MM-DD> \| generation <n> \| <finding ref> \| <reason>` with a resolvable `§`ref, a generation that is not previous-plus-one of the stamp generation, a re-stamp that drops a pending reopen's generation, a reopened section whose Implementation Order row is still `[x]`, or a still-stamped section with a reopened section anywhere in its Depends closure (the cascade is recursive: dependents park until the root re-stamps, bottom-up). A reopen voids the stamp everywhere downstream. |
| `stamp-generation-regressed` | FATAL | A persisted stamp generation below the HEAD-committed one for the same section: generations only increase, so a decrease means a hand edit stripped `generation <n>` off a post-lift stamp (or a re-verification forgot it), silently reviving old waivers. New sections and missing history skip. |
...
| `provenance-malformed` | FATAL | A post-cutoff findings file with no `Provenance:` line, or a provenance line outside the field shape (candidate, command, exit, tool, digest, path, run), without a shaped run ID, with a candidate naming no commit (blobs and trees fail; tags peel to their commit, and a recorded full ID must be the commit itself, not a tag object), with an absolute or NUL path, with a non-canonical path (`..` segments) or a path naming no regular file (missing, directory, symlink, gitlink) in the candidate tree on runs newer than 2026-09-19 (new records pin the primary reviewed artifact as a canonical path), with a path escaping the repo root or naming no file in the checkout before that, or with a run other than the section's last marker run. Unprovable git degrades to a warning, never a silent skip. The digest stays attested (presence plus shape, never re-verified). Fenced `Provenance:` examples strip before the scan. Stamps on or before 2026-09-18 predate the mandate and skip, per the §21 inventory (57 run-less pre-cutoff files, owner operator, deadline 2026-12-31). |
| `ledger-supersession-broken` | FATAL | A ledger row whose `supersedes <finding-id>` link names no row of its own block, crosses review namespaces, closes a cycle (self-links included), or re-supersedes an already-claimed target (one target, one successor: the second claim breaks the single-current-head read). Only exactly ID-shaped tokens link, so prose carrying the word is never a link. The link also proves its target with `identity <12hex>` (sha256 over the target row's normalized subject, severity, and disposition) and restates its reason (bare heads fire): severity plus disposition ride the grammar, deferred heads restate owner/due/trigger, filed heads name their own targets, and any disposition may head any target. plan-health reads the un-superseded head of each chain as current. No date scope: old ledgers deserve the same protection. |
| `risk-acceptance-malformed` | FATAL | A `Risk accepted:` line outside the record shape (target, id, approver, action owner, record date, expiry, review date, evidence commit, optional generation, supersedes link paired with its outcome, optional action, rationale), with a non-positive id, a supersedes link without its outcome (or an outcome without its link), with a target that names no finding ID, run ID, or `outage <rung> <date>`, or expiring before it is recorded, or carrying a review date outside its record-expiry window, or reviewing at expiry with no day of lead time, or citing evidence that resolves to no commit while git answers (fabricated). Fenced examples strip before the scan. Pre-cutoff records predate the mandate and are grandfathered. Coverage needs the record date on or before today and today on or before expiry, plus target-before-record, record generation equal to the section generation, evidence-commit freshness, and no superseder; a waiver for a minor row or healthy marker covers nothing (no escalation to terminate); an acceptance in a findings file attached only to pre-cutoff stamps is never validated and covers nothing. |
| `risk-acceptance-silent-edit` | FATAL | A `Risk accepted:` record edited or deleted against the committed history, or a `supersedes <ID>` link naming no record. Append-only is absolute: the predecessor stays byte-identical next to its successor, and a chain never licenses a rewrite. Amendments ride new records only, validated like ledger history; uncommitted findings skip. |
| `risk-acceptance-chain-broken` | FATAL | A `Risk accepted:` record that links to itself, sits in a supersedes cycle, claims one predecessor twice (one target, one successor), shares its id with a twin (one record per id), links across targets (succession stays same-target), or forks its target with a second unlinked record (one head per target: supersede or withdraw the twin). Mirrors the ledger supersession rule; needs no history, so uncommitted findings still fail. |
| `risk-acceptance-foreign-evidence` | FATAL | A `Risk accepted:` record whose evidence commit provably does not descend from the reviewed candidate (both peeled to full commits first): waivers attest the reviewed state or its descendants, never an unrelated branch or foreign history. Proven foreign only: candidate-less records, unpeelable IDs, and silent git skip without a sound, since the query fails closed at consult time. Peel-identity deliberately not required (D00 T04 §6 item 2 decides): tag-object evidence peels to its commit and proceeds; no reachable construction distinguishes it from commit evidence. |
| `duration-range-uncheckable` | FATAL | A `Duration:` range on a post-cutoff stamp that parses to no span (unshaped, calendar-invalid, or inverted), whose `Started:` anchor is outside the UTC-instant shape or not a real calendar instant, or whose start day misses the `Started:` day or whose end day misses the stamp day. Ranges anchor to the record's own dates, so arbitrary well-shaped instants cannot manufacture clearance ordering. Minute forms carry no instants and skip; pre-cutoff stamps keep the silent fallback. |
| `provenance-candidate-unprovable` | WARN | A provenance candidate git can neither resolve nor refuse (broken git, gitless export): the leg degrades visibly instead of skipping silently, and the warning blocks CI through the baseline until explicitly accepted. A working git that names nothing stays FATAL. |
| `provenance-short-candidate` | WARN | A provenance candidate recorded short in a run dated after 2026-09-19: new records resolve at mint time and record the full 40-hex ID, since shorts can ambiguate as history grows (they fail closed, so the ratchet warns rather than blocks). |
...
| `retired-term` | FATAL | A live markdown line contains a title in `RETIRED_TERMS`. Historical review files and the stamped §9 and §36 Why lines are pinned exclusions. The match is case-sensitive and literal. |
| `panel-slots` | FATAL | A panel-slot wiring break: `.conclave/panel.toml` is missing while skills name slots, or it fails to parse, names a model outside the closed set `PANEL_MODELS` in `scripts/panel_slots.py` or an effort outside `PANEL_EFFORTS`, misses one of the six governed slots (`bulk`, `signoff`, `depth`, `plan-primary`, `arch-primary`, `fallback`; D00 T04 §25), carries an ungoverned one (the retired fallback slots included), or breaks an effort-parity rule; or a review-skill command names a `--slot` the TOML does not carry, or a governed slot runs through no skill command, or pins a model or effort literally inside a fenced command block. Roots without the file skip: fixture trees predate the wiring. |
...
**The acceptance-outcome matrix, mirrored from `OUTCOME_MATRIX` in `scripts/todo-graph.py`: the self-test compares this table to the map row-for-row, so editing one without the other fails `self-test`.** A superseding record's outcome states the waiver's effect outright, so reporting an outcome never leaves coverage ambiguous. No outcome narrows; narrowing rides a fresh waiver with a narrower target plus a same-target superseder with outcome `closed` ending the broad scope (succession is same-target, so a cross-target superseder is chain-broken); the broad finding stays uncovered until its row resolves.
