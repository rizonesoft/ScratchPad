...
- `Plan review:` -- the second-family round's completion marker: which family ran it and the filings it produced, `no findings`, `outage: <rung> (owner <name>, due <YYYY-MM-DD>)` when both runners failed, `retry-owed (owner <name>, due <YYYY-MM-DD>)` on a same-family fallback run, or `partial: <rung>` when one rung failed and the other's findings stand (filings beside `partial:` are the survivor's). Lineage: every marker over a review record carries `(run <YYYYMMDD-DNN-TNN-SN-family[-rN]>)` minted by the `run-id` subcommand, the run's date prefix is its timestamp, the bare base is run 1 and `-rN` is run N for N >= 2; genesis is a singleton marker (run, no supersedes), and a rerun marker chains with `supersedes <prior-run>` or with `follows-outage` after an outage marker; runs never repeat within a section and the last run is one the manifest carries. A rerun opens a new `Plan review` record (rows number past the file max across records). Grammar: the last marker line governs; `no findings` never sits beside filings; `outage:` never sits beside filings, `no findings`, `retry-owed`, or `partial:`. Counts read in accepted findings (triaged ledger rows) versus implementation items (checklist lines); a merge states both numbers. Required on stamps dated after 2026-09-18; earlier stamps predate the rule.
...
| `plan-review-no-lineage` | FATAL | A `Plan review:` marker over a review record that carries no run ID, carries one outside the `YYYYMMDD-DNN-TNN-SN-family[-rN]` shape, reuses a run within its section, appends a rerun marker without `supersedes <prior-run>`, or names a run no manifest carries. Precedence without lineage rests on line position alone. Outage markers and markers over record-less findings are exempt. Stamps dated on or before 2026-09-18 skip lineage entirely; the rule postdates them. |
...
| `provenance-malformed` | FATAL | A post-cutoff findings file with no `Provenance:` line, or a provenance line outside the field shape (candidate, command, exit, tool, digest, path, run, semicolon-separated, no bare semicolons) or without a shaped run ID. Fenced `Provenance:` examples strip before the scan. Pre-cutoff records predate the mandate and are grandfathered. |
