...
| `stamp-no-plan-review` | FATAL | A stamp dated after 2026-09-18 that carries no `Plan review:` completion marker, or whose marker breaks the grammar. The second-family round runs after panel-close and its marker rides the stamp commit, naming the filings, `no findings`, `outage: <rung> (owner, due)`, or `retry-owed (owner, due)`; stamps on or before 2026-09-18 predate the rule and are grandfathered, so mechanical enforcement begins 2026-09-19 and landing-day stamps carry the marker voluntarily. The last marker line governs; `no findings` beside filings, and filings, `no findings`, or `retry-owed` beside `outage:`, are FATAL. Every named filing must resolve, and every ledger `filed` row's target must appear in that section's marker, checked per marker with no cross-section dedup (`outage:` markers skip both). |
| `plan-review-malformed` | FATAL | A post-cutoff plan-review record the query cannot parse: a `Plan review` section without its `Manifest:` line, a ledger-looking line outside the row shape, or a content-illegal row (a `deferred` row without owner, date, and trigger; a `duplicate` row naming no canonical finding; a `filed` row naming no target). Ledger-looking means a well-formed namespace-`PR` opener (no structure needed), an uppercase-`PR`-starting single token plus ledger structure (adjacent second bracket or `->`), or a single whitespace-free token containing `PR` with a digit/`§` tail plus structure (a mangled namespace, any case like the row pattern); lowercase pr-words stay prose even beside structure, and a bare `[PRn]` with no structure reads as prose. Pre-cutoff records predate the shapes and are grandfathered. |
...
| `stamp-reopened` | FATAL | A `> **Reopened:**` line outside `<YYYY-MM-DD> \| <finding ref> \| <reason>` with a resolvable `§`ref, a reopened section whose Implementation Order row is still `[x]`, or a still-stamped section depending on a reopened one (dependents park until it re-stamps). A reopen voids the stamp everywhere downstream. |
