...
| `risk-acceptance-malformed` | FATAL | A `Risk accepted:` line outside the record shape (target, approver, record date, expiry, review date, rationale), with a target that names no finding ID, run ID, or `outage <rung>`, or expiring before it is recorded. Fenced examples strip before the scan. Pre-cutoff records predate the mandate and are grandfathered. |
