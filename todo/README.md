...
| `outage-note-unlinked` | FATAL | An outage marker whose (rung, event-day) key resolves to no `Outage note: <rung> <YYYY-MM-DD>` line in the section's findings file, a note no marker keys, or a duplicated or malformed note key. Only the note carries the failure evidence, so an unlinked marker reads as recorded while supporting nothing. Chain-wide across superseded outage lines; matching is file-wide across sections sharing one findings file; fenced code blocks are stripped before the scan; stamps on or before 2026-09-20 predate the rule and are grandfathered. |
| `quorum-same-family` | FATAL | A clean or bare-partial primary review whose sign-off panel (last panel section) is the same family: quorum is one independent second-family pass, so a GPT review under a GPT sign-off panel met no quorum and owes `retry-owed` like any fallback survivor. Outage markers, `retry-owed` markers, and family-opaque markers stay silent under their own rules. Stamps on or before 2026-09-20 predate the rule and are grandfathered. |
