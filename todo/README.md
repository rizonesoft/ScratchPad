...
| `outage-note-unlinked` | FATAL | An outage marker whose (rung, event-day) key resolves to no `Outage note: <rung> <YYYY-MM-DD>` line in the section's findings file, a note no marker keys, or a duplicated or malformed note key. Only the note carries the failure evidence, so an unlinked marker reads as recorded while supporting nothing. Chain-wide across superseded outage lines; fenced code blocks are stripped before the scan; stamps on or before 2026-09-20 predate the rule and are grandfathered. |
