...
- `Plan review:` -- the second-family round's completion marker: which family ran it and the filings it produced, `no findings`, `outage: <rung>` when both runners failed, or `retry-owed` on a same-family fallback run. Required on stamps dated after 2026-09-18; earlier stamps predate the rule.
...
| `stamp-no-plan-review` | FATAL | A stamp dated after 2026-09-18 that carries no `Plan review:` completion marker. The second-family round runs after panel-close and its marker rides the stamp commit, naming the filings, `no findings`, `outage: <rung>`, or `retry-owed`; stamps on or before 2026-09-18 predate the rule and are grandfathered, so mechanical enforcement begins 2026-09-19 and landing-day stamps carry the marker voluntarily. Every named filing must resolve, and every ledger `filed` row's target must appear in that section's marker, checked per marker with no cross-section dedup (`outage:` markers skip both). |
| `plan-review-malformed` | FATAL | A post-cutoff plan-review record the query cannot parse: a `Plan review` section without its `Manifest:` line, or a ledger-looking line outside the row shape. Ledger-looking means a PR-ish opener (starts with `PR`/a namespace-`PR`, or a single whitespace-free token containing `PR` with a digit/`§` tail, any case like the row pattern) plus ledger structure later in the line (an adjacent second bracket or a `->`); a PR-ish label with no ledger tail (`proposal`, `prior art`, `prose`) is prose, as is any other bracket line (checkbox, citation, spaced label, `PR` without an ID tail). Pre-cutoff records predate the shapes and are grandfathered. |
