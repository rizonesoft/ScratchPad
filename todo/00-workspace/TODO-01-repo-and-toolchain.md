...
- [x] Live proofs stored as stable review artifacts (PR22). Done when: the skill routes live quotes into the findings file and the record carries them. Done: skill mandates a `Live proof` section in the findings file quoting each gate's command and output; this section's own record demonstrates it. **Corrected 2026-09-18 (review R1):** the record did not exist at ship; it lands in the R1 fix commit with Live proof quoting the build gates.
