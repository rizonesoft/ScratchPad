...
- [x] Prompt delimiters unforgeable from TODO text (PR18). Done when: the prompt templates use generated or escaped delimiters and a fixture proves injection fails. Done: both prompt templates generate per-prompt tags with a tagged-lines-only instruction; `review_prompt` fence/tag helpers; fixture proves hostile static delimiters carry no tag.
- [x] Malformed-output rule validates the whole output (PR19). Done when: the skill requires full-shape validation and the runner check implements it. Done: skill mandates `check-panel`/`check-plan` (FAIL is runner failure); CLI over stdin; fixtures lock trailing-garbage, missing-lens, repeat-lens, empty, and trailing-prose failures plus approves, details-under-finding, findings, and no-findings passes (real R5 output passes).
...
- [x] Live proofs stored as stable review artifacts (PR22). Done when: the skill routes live quotes into the findings file and the record carries them. Done: skill mandates a `Live proof` section in the findings file quoting each gate's command and output; this section's own record demonstrates it.
