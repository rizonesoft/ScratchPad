...
- [x] Manifest plus ledger shapes gain validator and self-test gates (malformed shapes FATAL with fixtures and a mutation probe). Done when: the rule plus README row plus cases are green (after the shape items land in this same section). Done: rule 18 plus `plan-review-malformed` class (date-scoped like rules 16-17); §6 proves missing-manifest, §8 proves malformed-row (PRX plus mis-namespaced); mutation (manifest removed) fails the §4 count check, reverted. **Corrected 2026-09-18 (reviews R1-R2):** R1 narrowed the trigger to PR-openers with prose decoys (checkbox, citation, bracket label stay silent) and gave the file defect first-reporter-wins dedup (§9's silence proves it); R2 caught mangled namespaces (`D00-T1x-S15-PR4` fires) with a case-sensitive-PR second alternative.
