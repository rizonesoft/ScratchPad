...
- [x] Self-test fixtures plus cases cover the rule: missing findings file, present file without the panel heading, panel heading with a missing lens verdict, pre-cutoff stamp without a panel (passes), a clean panel (passes), and both multi-round directions (stale-clean fires, stale-gap silent). Done when: each case asserts and the suite count is quoted. Done: 9 cases (class severity + 5 firing shapes + 3 silences), per-line assertions, fixtures unlinked after.
- [x] `self-test` green at the new count, the count in `AGENTS.md` updated to match, and live-tree `validate` still 0 fatal. Done when: all three are quoted. Done: 402 cases 0 failed; AGENTS.md says 402; live `validate` 0 fatal 0 warnings.
