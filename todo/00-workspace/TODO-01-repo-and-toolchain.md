...
- [x] Manifest plus ledger shapes gain validator and self-test gates (malformed shapes FATAL with fixtures and a mutation probe). Done when: the rule plus README row plus cases are green (after the shape items land in this same section). Done: rule 18 plus `plan-review-malformed` class (date-scoped like rules 16-17); §6 proves missing-manifest, §8 proves malformed-row (PRX plus mis-namespaced x2, six decoys silent); mutation (manifest removed) fails the §4 count check, reverted. **Corrected 2026-09-18 (reviews R1-R3):** R1 narrowed the trigger to PR-openers with prose decoys (checkbox, citation, bracket label stay silent) and gave the file defect first-reporter-wins dedup (§9's silence proves it); R2 caught mangled namespaces; R3 reworked the catch to single-token-plus-ID-tail in either case (R2's contains-PR shape false-fired on prose PR numbers and missed lowercase), with decoys for both, and the README row now states the trigger exactly.
