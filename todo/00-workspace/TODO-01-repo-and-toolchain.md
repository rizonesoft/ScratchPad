...
**Test checkpoint:** `python3 scripts/todo-graph.py self-test` green at the new quoted count with a case per residual; each new fixture is mutation-proven (revert its fix: §12/§13/§17 fail exactly their case; §15 fails with §12 on the shared anchored regex; §14/§16 fail with §9/§17 on the shared strip). Cheaper substitute that fails: a fourth scan patch without the re-think, or fixtures that pass vacuously.
