...
- [x] Clearance binds the fix commit or candidate hash plus timestamped verification, not just a same-file back-link and day ordering (PR10). Done when: the query proves the reviewed candidate contains the fix, with fixtures. Done: target sections name `fix <sha>` and the query confirms the commit's own tree contains the finding ID (plus the existing post-finding stamp and back-link legs). PR1 clears via canned `aaa1111`; PR17 (§21, back-linked and post-dated, `bbb2222` bytes lacking the ID) stays listed for the git leg alone.
