...
- [ ] The section's title-capture requirement and its foreground-funnel guarantee are reconciled with its background-delivery limitation into one acceptance contract, the losing text corrected in place. Done when: the section reads one title rule and one delivery guarantee, each with its fixture. (D00-T02-S30-PR10, from the D00 T02 §30 plan review.)
- [ ] The guarantee is narrowed to what register-then-unregister can prove (a chord free at probe time), or an execution boundary makes it enforceable (the press rides the probe window), stated in the section and `docs/testing.md`. Done when: the stated guarantee has a fixture that fails when it is violated. (D00-T02-S30-PR11.)
- [ ] The preflight manifest and bypass guard cover bare keys and every injection helper, not only modifier chords. Done when: a planted bare-key press through an alternate helper fails the guard. (D00-T02-S30-PR12.)
- [ ] A probe that errors (access or platform failure) reads `unknown` with its own remediation, distinct from a confirmed conflict's rebind advice. Done when: a probe-failure fixture reads unknown and names no rebind. (D00-T02-S30-PR13.)
...
- [ ] Model classifications are advisory: authoritative environment, rerun, and incident states win on conflict, and enrichment never changes gates, debt, ownership, or recovery. Done when: a fixture where the model disagrees with the incident state renders the incident state and marks the diagnosis advisory. (D00-T02-S30-PR14, from the D00 T02 §30 plan review.)
- [ ] Enrichment binds to an immutable run identity, candidate, and evidence snapshot hash, and a delayed result for another run is rejected rather than replacing the report or contradicting its toast. Done when: a stale-result fixture is rejected by hash. (D00-T02-S30-PR15.)
- [ ] Citations must reference identifiers inside the supplied snapshot, and the output accounts for every failure (diagnosed or listed as omitted). Done when: an invented citation and a silently dropped failure each fail validation. (D00-T02-S30-PR16.)
- [ ] Context size, historical sampling, and output volume are bounded by deterministic selection with omission counts shown in the block. Done when: a busy-night fixture renders within bounds and names how many failures it omitted. (D00-T02-S30-PR17.)
- [ ] One incident-centered block links each incident's recurrence, affected tests, outstanding proof, evidence, owner, due date, and next action (the ledger, the lifecycle JSON, and night debt in one place). Done when: the block renders for a fixture with two incidents and one open debt. (D00-T02-S30-PR18.)
