...
- [x] `tests/UI/UiLaunch.cs` (new) centralizes `LaunchApp`, `LaunchAppWithArgs`, `SeedSettings`, and `AppExePath` behind one helper, and every per-file copy (21 `LaunchApp` plus 23 `SeedSettings` at filing) delegates to it or is removed. Done when: definition greps for all four helpers return the one file. **Corrected 2026-09-20 (§11 validation):** was "both definitions"; the item centralizes four helpers, so all four greps must converge. Variance recorded: 4 files drain `LaunchDrops` inside `LaunchAppWithArgs` plus `SeedSettings` (JumpListTask, Launch, ProtocolHandler, TabAccessibility); the central pair takes an opt-in flag (default off: draining at launch would hide drops a later assert owns), the rest delegate plain. Done: 72 per-file defs removed (21/23/13/15), call sites re-qualified with drain flags at the 4 files, `SettingsPageTests` string-seed renamed `SeedSettingsFile` (different helper: returns the path, no session delete); definition grep for the four helper names returns `UiLaunch.cs` only (the `SeedSettingsFile` rename is a substring match, not a fifth helper); build 0 warnings 0 errors. Migration note: the first migrator pass mis-skipped files (path-separator basename bug), misrouting drain flags and `SettingsPage` calls plus `UiLaunch` itself; caught by count reconciliation before the build, repaired by fixup plus restore, verified by diff review of all 4 drain files.
