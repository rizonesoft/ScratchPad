...
- [x] A governed Run A re-proofs the gate clean (exit 0, primary=0) with the verdict quoted. Done when: the gate line reads exit 0 with primary=0. Done: attempt 4 `build/s11-runa-20260920-174727` gate exit 0 `flagged=0; census=648 primary=0 expect=secondary` plus suite 151 passed, 0 failed, 14 skipped.
...
- [x] Acceptance splits into reproduced-leak and no-longer-reproducible branches: the checkpoint names the leaking test only when the leak reproduces, else it quotes the clean re-verification runs. Done when: both branches read with their evidence. (D00-T02-S11-PR8.) Done: both branches read in the checkpoint; the no-repro branch taken with items 4 and 7 quoting the three clean runs.
- [x] Three consecutive governed Run A greens are owned with evidence rows, not just checkpoint prose: each run quotes its gate line plus trx path. Done when: three quoted gate lines read in the section. (D00-T02-S11-PR9.) Done: (1) attempt 2 `build/s11-runa-20260920-161048` gate exit 0 `primary=0` with suite 151/1/13 (the 1 quarantined per §5 as `settings-corrupt-dialog-null`); (2) attempt 3 `build/s11-runa-20260920-164204` gate exit 0 `primary=0` with suite 151/0/14; (3) attempt 4 `build/s11-runa-20260920-174727` gate exit 0 `primary=0` with suite 151/0/14. All three run the fixed-filter population (`Category!=Interactive&Category!=Primary`); trx at each `run-a.trx`.
- [x] Each governed run captures a manifest (build identity, settings snapshot, monitor topology, DPI, session, harness version) so nondeterminism compares instead of guesses. Done when: a manifest is quoted beside its gate line. (D00-T02-S11-PR11.) Done: attempt-4 manifest beside its gate line in item 7: build `UI.dll` from the §12-stamped tree (`31b0906`; §13 filter fix uncommitted at run time, mirror script carries the same filter); settings per-test `SeedSettings` (defaults 50/50 to off-screen under `SCRATCHPAD_BACKGROUND=1`, Primary excluded); topology DISPLAY1 2560x1440 primary plus DISPLAY2 1920x1080 at -1920,1080; DPI primary 150% device (unaware-context API reads 96), secondary 96; session `AzureAD\DerickPayne` console; harness dotnet 10.0.400 plus ForegroundLog gate plus xUnit.
