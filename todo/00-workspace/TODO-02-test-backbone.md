...
- [ ] Re-prove the full UI suite on a Windows host and quote the next CI `build-windows` job green with zero golden-failure artifacts. Done when: both greens are quoted with the run id. **Corrected 2026-09-17:** also re-prove the neutral scope green on Linux locally and quote CI `build-linux` green: the section's Why ("no push proves itself") is false while either job is red.
...
- [ ] Commit: `"workspace: make CI suites green and deterministic"` (**Corrected 2026-09-17:** was `"workspace: make golden comparison deterministic on CI"`; the scope now covers all three red signatures per the fold-in rule.)
