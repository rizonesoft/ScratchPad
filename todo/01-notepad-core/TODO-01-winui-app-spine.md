...
**Corrected 2026-09-14:** the seed named `src/Notepad/MainWindow.xaml`, `tests/UI/MainWindowTest`, and `resources/baseline/main-window/`; the app project is `src/ScratchPad/` (D00 T01 §2), the test file follows the `*Tests` convention, and the §3 store is flat with `stock/notepad-main-n11.2607.14.0-win25h2.png` as the main-window capture. Items 3 and 5 persist through a local seam the D01 T02 §2 store adopts, since the store ships later in this phase.

- [x] `src/ScratchPad/MainWindow.xaml` hosts the menu bar region, tab region, editor region, and status region. Done when: all four regions exist with the Notepad layout. Measured chrome row heights (43/32 DIP) land with §27 here.
...
- [x] `src/ScratchPad/TabBar.xaml` renders the `TabModel` list with new-tab, switch, and close, and no-reorder parity. Done when: every gesture in the capture works and a drag attempt leaves the order unchanged. Measured chrome geometry (caption inset, zero-tab centering, 6-DIP dot) lands with §27 here.
...
- [ ] `LargeFileOpensResponsively` (D01 T01 §8, `large-file-attach-uia-timeout`) is fixed at the cause (the UIA attach timing out while the 4 MB open holds the provider under suite load), or quarantined by the D00 T02 §5 procedure and then fixed or removed inside its window. Done when: the cause is quoted and a full default UI run plus a soak pass with the test un-skipped, or the quarantine row is quoted and later closed by fix or removal decision. -> SOURCE: full-default-run-flake-2026-09-25 (`System.TimeoutException: UIA Timeout`, inner COMException 0x80131505, from `UIA3Automation.FromHandle` in `UiApp.Attach` (`tests/UI/UiApp.cs:22`, call site `LaunchTests.cs:422`); commit `5144d27` red once in `dotnet test src/ScratchPad.slnx --filter "Category!=Interactive&Category!=Primary" -e SCRATCHPAD_BACKGROUND=1` (UI.dll 250 passed, 1 failed, 14 skipped), then green 3/3 on isolated reruns of the same build (3 s each); same attach-timeout family as `chrome-icon-uia-timeout` and `dirty-close-com-timeout`.)
