---
name: groom-plan
description: Grok campaign runner. Harden the todo tree for a weaker executor. Sequence check, drift sweep, gap scan, complete-feature pass. Never move rows between phases, tick boxes, or start a campaign. Use before a long run, or when the plan feels stale.
---

# Groom Plan

Grok runner. Do not open `.claude/skills/groom-plan`. Host rules: `.grok/rules/campaign-runner.md`.
...
Run `python3 scripts/todo-graph.py validate` after every structural edit. Cycles are FATAL: break them at authoring time.
...
Check the Adjacency declarations while here: `python3 scripts/todo-graph.py query adjacency`. Advisories that name real gaps become sections. Ones that are already covered get their anchors sharpened.

### 4. Complete-feature pass

For every UI surface in the plan, confirm the feature is whole: list, find, create, edit, delete (or the honest not-applicable), settings with consumers, permissions both ways, and the reverse of every create. For every protocol surface, confirm both directions, cancellation, timeouts, and fault survival. File what is missing. Do not redesign what exists.
