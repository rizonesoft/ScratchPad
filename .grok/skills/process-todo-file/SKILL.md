...
description: Grok campaign runner. Close out a TODO file whose sections are all shipped. Sweep for loose ends, run the file-level Verification block, reconcile deferrals, update the indexes, and set status to done. Use when every Implementation Order row is [x], or when asked to close a whole TODO file.
---

# Process TODO File

Grok runner. Do not open `.claude/skills/process-todo-file`. Host rules: `.grok/rules/campaign-runner.md`.
...
Do not use it to force a file closed. If sections remain open, they get implemented or explicitly deferred with owners.
...
Interactive tests skip outside 02:00-06:50 local. A daytime full suite that is green with those skips is the honest result. Do not force them. Quote the skip count. The nightly collector owns the proof. See `.grok/skills/process-todo-section/gates.md`.

Also run the file's other Verification items (warnings, analysis, captures, clean-machine checks), each executed, none trimmed.

### 3. Loose-end sweep

Read the whole file with fresh eyes and check:

- **Stubs and TODOs in the code.** Search the touched paths for `TODO`, `FIXME`, and any not-implemented marker introduced during the work. Each is either finished now, or gets an owning section and an XREF.
...
- **One-sided XREFs.** The validator warns on these. Resolve them.
- **Integration reality.** Is the feature reachable from where a user would look for it? Wired end to end, not just present? A surface nobody can navigate to is not done. A control whose job is a write and that only lists is not done either: the user must be able to change the value, and a consumer must read it.
...
- Golden-output fixtures are committed, not left in a scratch directory.
...
Then sync the plan: `python3 scripts/todo-graph.py plan --sync`.

### 7. Report

Tell the user plainly:

- What now works, in their terms.
